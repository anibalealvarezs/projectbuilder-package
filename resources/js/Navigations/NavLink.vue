...
export default {
    ...
    computed: {
        liclasses() {
            let classes = 'dropdown h-full inline-block relative hover:bg-gray-100 text-sm font-medium leading-5 transition duration-150 ease-in-out focus:outline-none'
        ...
        isActive() {
            if (this.active) {
                return ' border-indigo-400 text-gray-900 focus:border-indigo-700'
            } else {
                return ' border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:text-gray-700 focus:border-gray-300'
            }
        },

        aclasses() {
            let classes = 'items-center h-full sm:flex px-3 border-b-2 hover:ring-0'
            return classes
        },
